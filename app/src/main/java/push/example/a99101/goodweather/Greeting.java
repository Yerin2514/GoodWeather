...
public class Greeting {
    ...
    public String getCorrectGreeting() {
        ...
        if (percip > 60) {
            correctGreeting = correctGreeting.concat("\n비 올 확률 있음");//It's also looking like it might rain.
        }
    ...
    private String addToGreeting(int element, int choice) {
        ...
        switch (choice) {
            case 1:
                result =greetings.get(element).concat("매우 추움");  //It's really cold outside
                break;
            case 2:
                result = greetings.get(element).concat("쌀쌀함");  //It's a bit chilly outside.
                break;
            case 3:
                result = greetings.get(element).concat("맑음");  //Its' feeling rather nice out
                break;
            case 4:
                result = greetings.get(element).concat("더움");   //It's really hot outside
                break;
        }
        return result;
    }
